(function() {
  ...
  Ship.prototype.draw = function (ctx) {
    ...
    if ( this.vel[0] > 0 ) {
      var ship_angle = Math.atan(this.vel[1] / this.vel[0]) + Math.PI/180 * 45;
    } else {
      var ship_angle = Math.atan(this.vel[1] / this.vel[0]) - Math.PI/180 * 135;
    }
    
    ctx.rotate(ship_angle);
    ctx.drawImage(Asteroids.ship_img, -15, -15, 30, 30);
    ctx.restore();
  }
